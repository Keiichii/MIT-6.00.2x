...
class Patient(object):
    ...
    def update(self):
        ...
        # Current population density calculating
        density = self.getTotalPop() / self.getMaxPop()
        # Reproducing
        for virus in self.getViruses().copy():
            try:
                self.getViruses().append(virus.reproduce(density))
...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for trial in range(numTrials):
        ...
        for timestamp in range(300):
            patient.update()
            if average.get(timestamp):
                average[timestamp] += patient.getTotalPop()
            else:
                average[timestamp] = patient.getTotalPop()
    for k,v in average.items():
        average[k] = v / numTrials
    pylab.plot(list(average.values()), label = "SimpleVirus")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        for drug in activeDrugs:
            check = check and self.isResistantTo(drug)
        if check:
            if random.random() < self.maxBirthProb * (1 - popDensity):
                for resist in self.getResistances():
                    if random.random() < self.getMutProb():
                        if self.getResistances()[resist]:
                            self.getResistances()[resist] = False
                        else:
                            self.getResistances()[resist] = True
